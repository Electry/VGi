...
static int g_hook_deref_register = 0;
static uint32_t g_hook_deref_offset = 0;
...
static uint32_t _hook_dump_deref(uint32_t R0, uint32_t R1, uint32_t R2, uint32_t R3, uint32_t stack) {
    uint32_t SP = (uint32_t)&stack;
    uint32_t LR, S[4];
    asm volatile ("MOV %0, LR\n" : "=r" (LR));
    ...
    uint32_t deref = 0;
    switch (g_hook_deref_register) {
    default: deref = 0; break;
    case 0: deref = *(uint32_t *)(R0 + g_hook_deref_offset); break;
    case 1: deref = *(uint32_t *)(R1 + g_hook_deref_offset); break;
    case 2: deref = *(uint32_t *)(R2 + g_hook_deref_offset); break;
    case 3: deref = *(uint32_t *)(R3 + g_hook_deref_offset); break;
    }
    ...
    snprintf(res_msg, sizeof(res_msg), "0x%08X:  RET: 0x%lX\n"
                                       ...
                                       "    SP:  0x%08lX   LR:  0x%08lX\n",
                                       g_hook_vaddr, ret,
                                       g_hook_deref_register, g_hook_deref_offset, deref,
                                       R0, R1, R2, R3,
                                       S[0], S[1], S[2], S[3],
                                       SP, LR);
    vgi_cmd_send_msg(res_msg);
...
void vgi_cmd_hook_dump_deref_hook(char *arg) {
    ...
    g_hook_dump_stack = true;

    uint32_t pos = 0;
    while (isspace(endptr[pos])) pos++;

    // Read deref register
    g_hook_deref_register = strtoul(&endptr[pos], &endptr2, 0);
    if (endptr2 == &endptr[pos])
        g_hook_deref_register = 0;

    pos += (endptr2 - &endptr[pos]);
    while (isspace(endptr[pos])) pos++;

    // Read deref offset
    g_hook_deref_offset = strtoul(&endptr[pos], &endptr2, 0);
    if (endptr2 == &endptr[pos])
        g_hook_deref_offset = 0;
    ...
    g_hook_antiflood = 0;
    g_hook_vaddr = addr;
    g_hook_hookid = taiHookFunctionOffset(&g_hook_hookref, g_app_tai_info.modid, 0, offset, 1, _hook_dump_deref);
    snprintf(res_msg, RES_MSG_LEN, "Hook offset 0x%lX, UID: 0x%X, deref R%d+0x%lX\n", offset, g_hook_hookid, g_hook_deref_register, g_hook_deref_offset);
    vgi_cmd_send_msg(res_msg);
}
